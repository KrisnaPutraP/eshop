...
- Single Responsibility Principle. I've separated all of the Controller file according to their endpoint. `MainController.java` is responsible for `/` endpoint, `CarController.java` is responsible for `/car` endpoint, and `ProductController.java` is responsible for `/product` endpoint. They all are subclasses of `AbstractCrudController` that I explain in the next principle.
- Open Close Principle. I've created abstract class inside the controller so that all of my controller classes are subclass of it, hence its open for extension. But for the methods, its all declared inside the `AbstractCrudController` so that the subclasses like `CarController` and `ProductController` just need to override the methods without having to modify the functionality.
